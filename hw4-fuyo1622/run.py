# DVGO training and rendering entry point adapted for the HW4 hotdog scene.
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
                      gt_imgs=None, savedir=None, dump_images=False, output_dir=None,save_paths=None,
    ...
    if output_dir is not None and dump_images:
        for i in trange(len(rgbs)):
            rgb8 = utils.to8b(rgbs[i])
            filename = os.path.join(output_dir, save_paths[0][i]+".png")
...
def load_everything(args, cfg):
    ...
    '''
    data_dict = load_data(args.test_json_path,cfg.data)

    # remove useless field
    kept_keys = {
            'hwf', 'HW', 'Ks', 'near', 'far', 'near_clip',
            'i_train', 'i_val', 'i_test', 'irregular_shape',
            'poses', 'render_poses', 'images', 'save_paths'}
...
if __name__=='__main__':
    ...
    if args.render_test:
        ...
        rgbs, depths, bgmaps = render_viewpoints(
                ...
                savedir=testsavedir, dump_images=args.dump_images, output_dir=output_dir, save_paths=data_dict['save_paths'],
                eval_ssim=args.eval_ssim, eval_lpips_alex=args.eval_lpips_alex, eval_lpips_vgg=args.eval_lpips_vgg,
                **render_viewpoints_kwargs)
        imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=30, quality=8)
        imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=30, quality=8)
